...
def get_user_by_id(user_id):
    ...
    try:
        ...
        if data:
            ...
            data=conn.execute('SELECT first_name, last_name,username,email FROM user where userID = ?',(user_id,)).fetchone()
            return dict(data),200
    except:
...
def delete_account(user_id):
    ...
    try:
        ...
        if user == int(user_id):
            ...
            conn.execute('DELETE FROM user_details WHERE userID=?',(user_id,))
            conn.commit()
